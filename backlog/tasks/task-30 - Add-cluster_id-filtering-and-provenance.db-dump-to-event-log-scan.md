...
<!-- SECTION:NOTES:BEGIN -->
Reworked condor_history's schema (db.py): composite PRIMARY KEY (cluster_id, proc_id) -- fixing the identical job-array bug just fixed in event_log_scan.py, since condor_history returns one ClassAd per proc, not per cluster -- plus new proc_id/job_name/site/gpu_ids/status/source columns. history_enrich.py now maps ProcId, tags every row source='condor_history', derives a status string from the numeric JobStatus ClassAd attribute via a small lookup table, and fixes the already-enriched check to scope on source='condor_history' specifically (not "any row exists"). Added write_scan_records()/_row_from_scan_record() mapping event_log_scan.py's record shape onto the same table with source='event_log'.

Follow-up fix after initial review: the first version of _upsert_history_row used INSERT OR REPLACE, which blindly replaces the whole row -- writing from one source after the other had already written the same (cluster_id, proc_id) would silently NULL out whichever columns only the first write populated. Reworked to a real merge: ON CONFLICT DO UPDATE with COALESCE(excluded.col, condor_history.col) per column, classad JSON split into separate condor_history_json/event_log_json columns (each nullable, only ever written by its own source), and `source` unioned into a sorted comma-set (idempotent -- writing the same source twice doesn't duplicate it) rather than overwritten. Also fixed enrich_from_condor_history's "already enriched" check, which previously matched ANY row regardless of source -- a cluster_id touched only by scan --db was being treated as already condor_history-enriched and silently skipped.

event_log_scan.py's scan_event_log() gained a cluster_ids allowlist parameter, filtering at block-flush time so non-matching clusters are never even parsed into records; every proc of a matched cluster is kept. Wired as repeatable `--cluster-id` and `--db <path>` on `mldag-query scan`.
...
Verified end-to-end against rocksrocksrocks.log: --cluster-id filtering, --db dump, and idempotent re-scan (source set doesn't grow on repeat writes from the same source) all behave correctly. 12 new tests (3 in test_event_log_scan.py for filtering, 9 in test_history_enrich.py for job arrays/source-tagging/write_scan_records/merge-not-replace semantics). Full suite: 274 passed, 2 skipped; ruff clean; ty clean except the two pre-existing build_database diagnostics and the expected htcondor2 unresolved-import.
<!-- SECTION:NOTES:END -->
